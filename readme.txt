github URL: You ae already here, I don;t know why I have this line
...
Features: Search for all cards without having to specify them completely! This bot can find cards even if the punctuation is not correct, which is a pain for some cards (e.g.  Abu Ja'far). Finally, the saves its info in persistant storage, so if it errors and shuts down, there is a good chance it can be restarted without fear of it double posting a lot.
...
Sources: Much of the framework logic is taken from the praw tutorial linked in the 470Z subreddit. For JSON, I looked heavily at examples given in the docs.python.org JSON section. The card database (not included in the repository because if size) was obtained from mtgjson.com. The card images are hosted by mtgimage.com. 
